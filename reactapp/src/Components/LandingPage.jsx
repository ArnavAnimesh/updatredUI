...
const LandingPage = () => {
    ...
    const getRootClass = () => {
        if (theme === 'osmo') return "min-h-screen bg-[#fafafa] text-[#0f0f0f] font-sans overflow-x-hidden page-transition";
    ...
    const getNavClass = () => {
        if (theme === 'osmo') return "fixed top-0 w-full z-50 bg-[#fafafa]/90 backdrop-blur-md border-b border-[#f0f0f0] px-8 py-4 flex justify-between items-center transition-colors duration-300";
    ...
    const getSignInBtnClass = () => {
        if (theme === 'osmo') return "text-xs font-bold uppercase tracking-widest text-[#0f0f0f]/70 hover:text-[#0f0f0f] hover:bg-black/5 transition-all px-6 py-3 rounded-full";
    ...
    const getSignUpBtnClass = () => {
        if (theme === 'osmo') return "bg-[#0f0f0f] hover:bg-[#1a1a1a] text-white text-xs font-bold uppercase px-6 py-3 rounded-full transition-all shadow-sm active:scale-95";
    ...
    return (
        <div className={getRootClass()}>
            ...
            <section className="relative pt-32 pb-20 px-8 flex flex-col items-center text-center max-w-7xl mx-auto">
                ...
                <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-xs font-bold uppercase tracking-widest mb-8 animate-bounce ${theme === 'gravity' ? 'bg-amber-500/10 border border-amber-500/20 text-amber-500' : theme === 'osmo' ? 'bg-indigo-50 border border-indigo-100 text-indigo-600' : 'bg-orange-50 border border-orange-100 text-[#F97316]'}`}>
                ...
                <h1 className={`text-6xl md:text-8xl font-black tracking-tight leading-[1.1] mb-8 ${theme === 'gravity' ? 'text-white' : 'text-slate-900'}`}>
                    Where Big Ideas <br /> 
                    <span className={`text-transparent bg-clip-text bg-gradient-to-r ${theme === 'gravity' ? 'from-purple-500 to-amber-500 drop-shadow-[0_0_15px_rgba(124,58,237,0.3)]' : theme === 'osmo' ? 'from-indigo-600 to-indigo-400' : 'from-[#F97316] to-orange-400'}`}>Take Flight.</span>
                </h1>
                ...
                <div className="flex flex-col sm:flex-row gap-6">
                    ...
                    <button 
                        ...
                        className={theme === 'osmo' ? "bg-white border border-[#e4e4e7] hover:border-black hover:shadow-md text-black px-10 py-5 rounded-full text-lg font-bold transition-all active:scale-95 hover:-translate-y-1" : theme === 'gravity' ? "bg-white/5 border border-white/10 hover:bg-white/10 text-white px-10 py-5 rounded-xl text-lg font-bold transition-all backdrop-blur-md active:scale-95" : "bg-white border-2 border-slate-100 hover:border-slate-200 text-slate-800 px-10 py-5 rounded-2xl text-lg font-black uppercase transition-all active:scale-95"}
                    >
                        Learn More
                    </button>
                </div>
            ...
            <section className={`py-24 px-8 ${theme === 'gravity' ? 'bg-[#050510]' : theme === 'osmo' ? 'bg-[#f4f4f5] border-y border-[#e4e4e7]' : 'bg-slate-50 border-y border-slate-100'}`}>
                <div className="max-w-7xl mx-auto">
                    <div className="text-center mb-20">
                        <h2 className={`text-4xl tracking-tight ${theme === 'gravity' ? 'text-white font-bold' : theme === 'osmo' ? 'text-[#0f0f0f] font-bold' : 'text-slate-900 font-black uppercase'}`}>Dual-Role Ecosystem</h2>
                        <div className={`w-20 h-1.5 mx-auto mt-4 rounded-full ${theme === 'gravity' ? 'bg-purple-600 shadow-[0_0_10px_rgba(124,58,237,0.5)]' : theme === 'osmo' ? 'bg-indigo-500' : 'bg-[#F97316]'}`}></div>
                    </div>
            ...
            <section className="py-24 px-8 text-center max-w-5xl mx-auto">
                <div className={`${theme === 'gravity' ? 'bg-white/5 backdrop-blur-2xl border border-white/10 shadow-[0_0_40px_rgba(124,58,237,0.2)] rounded-[2rem]' : theme === 'osmo' ? 'bg-white border border-[#e4e4e7] shadow-xl rounded-[2rem]' : 'bg-gradient-to-br from-[#1E3A5F] to-[#2D5282] rounded-[3rem] shadow-2xl'} p-16 relative overflow-hidden`}>
                    {/* Decorative Icon */}
                    <RiHandCoinLine className={`absolute -bottom-10 -right-10 text-[20rem] rotate-12 ${theme === 'gravity' ? 'text-purple-500/10' : theme === 'osmo' ? 'text-indigo-500/5' : 'text-white/5'}`} />
                    
                    <h2 className={`text-4xl md:text-5xl font-black mb-8 tracking-tight ${theme === 'osmo' ? 'text-[#0f0f0f]' : 'text-white'}`}>
                        Ready to hatch your <br /> next big idea?
                    </h2>
                    <button 
                        onClick={() => navigate('/signup')}
                        className={theme === 'osmo' ? "bg-[#0f0f0f] hover:bg-[#1a1a1a] text-white px-12 py-6 rounded-full text-xl font-bold transition-all shadow-lg active:scale-95 hover:-translate-y-1 relative z-10" : theme === 'gravity' ? "bg-purple-600 hover:bg-purple-700 text-white px-12 py-6 rounded-xl text-xl font-bold transition-all shadow-[0_0_20px_rgba(124,58,237,0.4)] active:scale-95 relative z-10 btn-glow" : "bg-[#F97316] hover:bg-[#EA6C0A] text-white px-12 py-6 rounded-2xl text-xl font-black uppercase transition-all shadow-2xl shadow-orange-500/40 active:scale-95 relative z-10"}
                    >
                        Join StartupNest Today
                    </button>
                    <p className={`mt-8 font-bold text-sm tracking-widest uppercase italic relative z-10 ${theme === 'osmo' ? 'text-gray-400' : 'text-white/40'}`}>
            ...
            {/* 5. FOOTER */}
            <footer className={`py-12 border-t text-center font-bold text-xs uppercase tracking-widest ${theme === 'gravity' ? 'border-white/5 text-gray-600' : theme === 'osmo' ? 'border-[#e4e4e7] text-gray-400' : 'border-slate-100 text-slate-400'}`}>
