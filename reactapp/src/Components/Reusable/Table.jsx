...
const Table = ({ columns, rows, renderRow }) => {
    ...
    const getContainerClass = () => {
        ...
        if (theme === 'osmo') return "w-full overflow-hidden overflow-x-auto border border-[#f0f0f0] rounded-[1.5rem] shadow-sm bg-white";
    ...
    const getHeaderRowClass = () => {
        ...
        if (theme === 'osmo') return "bg-transparent";
    ...
    const getHeaderCellClass = () => {
        ...
        if (theme === 'osmo') return "sticky top-0 z-10 bg-[#fafafa] px-6 py-5 text-xs font-bold tracking-widest text-[#71717a] uppercase border-b border-[#f0f0f0]";
    ...
    const getRowClass = () => {
        ...
        if (theme === 'osmo') return "transition-all duration-200 hover:bg-[#fafafa] border-b border-[#f0f0f0] last:border-b-0";
    ...
    return (
        <div className={getContainerClass()}>
            <table className="w-full text-left border-collapse min-w-[800px]">
                ...
                <tbody className={theme === 'gravity' || theme === 'osmo' ? '' : 'divide-y divide-gray-50'}>
                    {rows && rows.length > 0 ? (
